...
ARCH=$arch
VER=v0.35.1
wget https://github.com/bytecodealliance/wasmtime/releases/download/${VER}/wasmtime-${VER}-${ARCH}-${OS_NAME}-c-api.tar.xz
tar -xvf ./wasmtime-${VER}-${ARCH}-${OS_NAME}-c-api.tar.xz > /dev/null
if [ -d wasmtime-c-api ]; then
    rm -rf wasmtime-c-api
fi
mv wasmtime-${VER}-${ARCH}-${OS_NAME}-c-api wasmtime-c-api
